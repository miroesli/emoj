...
def display_options(room_uid, player_uid, selected):
    ...
    if len(selected.get('locations')) > 1:
        ...
        location_cards = db_handler.get_location_cards(room_uid,selected.get('locations')[0])
        ...
        #non empty location selected
            ...
            if len(selected.get('players')) > 0:
                pass # deal that player a card(for each revealed) + deal player X hidden cards
            # only non empty location selected
            else:
                pass # draw a card(for each revealed) + draw player X hidden cards
    #zero locations
    else:
        # player selected
        if len(selected.get('players')) > 0:
        ...
        # only cards are selected
            ...
            for card in selected.get('cards'):
                if not card.revealed:
                    result.append({"option": "reveal","card_uid":card.card_uid,"option_text":"reveal "+ card.card_name})
            return result
